...
#------------------------
#do the long-term traffic flow prediction (24h)
start_long <- which(new_flow$timeStamp == '2019-11-22 00:00:00')
end_long <- which(new_flow$timeStamp == '2019-12-27 23:45:00')
yLong <- new_flow[start_long:nrow(new_flow),'newFlow',drop=FALSE]
colnames(yLong) <- 'y' 
row.names(yLong) <- NULL  #reset the index
xLong <- data.frame(embed(x = new_flow[1:(end_long+3), 'newFlow'], dimension = m+1))
colnames(xLong) <- c("y_t-21","y_t-22","y_t-23","y_t-24")
class(yLong)

dataLong <- data.matrix(cbind(yLong, xLong))
...
yTrain <- dataLong[1:split,1]
XTrain <- dataLong[1:split,-1]
yTest <- dataLong[(split+1):nrow(dataLong),1]
XTest <- dataLong[(split+1):nrow(dataLong),-1]
...
# refine the grid to see if we can gain further improvements in performance:
SVRGridFine <- expand.grid(.sigma=c(0.05, 0.01, 0.015), .C=c(500,1000,1500))
system.time(SVRFitFine <- train(XTrain, yTrain, method="svmRadial", tuneGrid=SVRGridFine, 
                                trControl=ctrl, type="eps-svr"))
...
#use the model for one-step-ahead prediction and plot the results
yPred <- predict(SVRFitCoarse, XTest)
plot(yTest, type="l", xaxt="n", xlab="Date", ylab="UKTemp")
lines(yPred, col="red", lwd=2)
points(yPred, col="blue", pch=21, bg="blue")
#axis(1, at=seq(90, (180*5)+90, 180), labels=unique(dates[(split+1):nrow(data),1]))
